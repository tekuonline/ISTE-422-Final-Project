...
public class EdgeConvertFileParser {
	...
	public static final String EDGE_ID = "EDGE Diagram File"; // first line of
																// .edg files
																// should be
																// this
	public static final String XMI_ID = "XMI Diagram File"; // first line of
															// .xmi files should
															// be this
	public static final String SAVE_ID = "EdgeConvert Save File"; // first line
																	// of save
																	// files
																	// should be
																	// this
	...
	public void parseEdgeFile() throws IOException {
		while ((currentLine = br.readLine()) != null) {
			...
			if (currentLine.startsWith("Figure ")) { // this is the start of a
														// Figure entry
				numFigure = Integer.parseInt(currentLine.substring(currentLine.indexOf(" ") + 1)); // get
																									// the
																									// Figure
																									// number
				currentLine = br.readLine().trim(); // this should be "{"
				currentLine = br.readLine().trim();
				if (!currentLine.startsWith("Style")) { // this is to weed out
														// other Figures, like
														// Labels
					continue;
				} else {
					style = currentLine.substring(currentLine.indexOf("\"") + 1, currentLine.lastIndexOf("\"")); // get
																													...
																													// Style
																													// parameter
					if (style.startsWith("Relation")) { // presence of Relations
														// implies lack of
														// normalization
					...
					if (escape > 0) { // Edge denotes a line break as "\line",
										// disregard anything after a backslash
						text = text.substring(0, escape);
					}

					do { // advance to end of record, look for whether the text
							// is underlined
						currentLine = br.readLine().trim();
						if (currentLine.startsWith("TypeUnderl")) {
							isUnderlined = true;
						}
					} while (!currentLine.equals("}")); // this is the end of a
														// Figure entry

					if (isEntity) { // create a new EdgeTable object and add it
									// to the alTables ArrayList
						if (isTableDup(text)) {
					...
					if (isAttribute) { // create a new EdgeField object and add
			...
			if (currentLine.startsWith("Connector ")) { // this is the start of
														// a Connector entry
				numConnector = Integer.parseInt(currentLine.substring(currentLine.indexOf(" ") + 1)); // get
																										// the
																										// Connector
																										// number
				currentLine = br.readLine().trim(); // this should be "{"
				...
				currentLine = br.readLine().trim(); // not interested in
													// EndPoint1
				currentLine = br.readLine().trim(); // not interested in
													// EndPoint2
				currentLine = br.readLine().trim(); // not interested in
	...
	private void resolveConnectors() { // Identify nature of Connector endpoints
		...
		for (int cIndex = 0; cIndex < connectors.length; cIndex++) {
			...
			fieldIndex = -1;
			for (int fIndex = 0; fIndex < fields.length; fIndex++) { // search
																		// fields
																		// array
																		// for
																		// endpoints
				if (endPoint1 == fields[fIndex].getNumFigure()) { // found
																	// endPoint1
																	// in fields
																	// array
					connectors[cIndex].setIsEP1Field(true); // set appropriate
															// flag
					fieldIndex = fIndex; // identify which element of the fields
											// array that endPoint1 was found in
				}
				if (endPoint2 == fields[fIndex].getNumFigure()) { // found
																	// endPoint2
																	// in fields
																	// array
					connectors[cIndex].setIsEP2Field(true); // set appropriate
															// flag
					fieldIndex = fIndex; // identify which element of the fields
											// array that endPoint2 was found in
				}
			}
			for (int tIndex = 0; tIndex < tables.length; tIndex++) { // search
																		// tables
																		// array
																		// for
																		// endpoints
				if (endPoint1 == tables[tIndex].getNumFigure()) { // found
																	// endPoint1
																	// in tables
																	// array
					connectors[cIndex].setIsEP1Table(true); // set appropriate
															// flag
					table1Index = tIndex; // identify which element of the
											// tables array that endPoint1 was
											// found in
				}
				if (endPoint2 == tables[tIndex].getNumFigure()) { // found
																	// endPoint1
																	// in tables
																	// array
					connectors[cIndex].setIsEP2Table(true); // set appropriate
															// flag
					table2Index = tIndex; // identify which element of the
											// tables array that endPoint2 was
											// found in
				}
			}

			if (connectors[cIndex].getIsEP1Field() && connectors[cIndex].getIsEP2Field()) { // both
																							// endpoints
																							// are
																							// fields,
																							// implies
				...
				EdgeConvertGUI.setReadSuccess(false); // this tells GUI not to
														// populate JList
														// components
				break; // stop processing list of Connectors
			}

			if (connectors[cIndex].getIsEP1Table() && connectors[cIndex].getIsEP2Table()) { // both
																							// endpoints
																							// are
																							// tables
				if ((connectors[cIndex].getEndStyle1().indexOf("many") >= 0)
						&& (connectors[cIndex].getEndStyle2().indexOf("many") >= 0)) { // the
																						// connector
																						// represents
																						// a
																						// many-many
					...
					EdgeConvertGUI.setReadSuccess(false); // this tells GUI not
															// to populate JList
															// components
					break; // stop processing list of Connectors
				} else { // add Figure number to each table's list of related
							// tables
			...
			if (fieldIndex >= 0 && fields[fieldIndex].getTableID() == 0) { // field
																			// has
																			// not
																			// been
																			// assigned
																			...
																			// table
																			// yet
				if (connectors[cIndex].getIsEP1Table()) { // endpoint1 is the
															// table
					tables[table1Index].addNativeField(fields[fieldIndex].getNumFigure()); // add
																							// to
																							// the
																							// appropriate
																							// table's
																							// field
																							// list
					fields[fieldIndex].setTableID(tables[table1Index].getNumFigure()); // tell
																						// the
																						// field
																						// what
																						// table
					...
					tables[table2Index].addNativeField(fields[fieldIndex].getNumFigure()); // add
																							// to
																							// the
																							// appropriate
																							// table's
																							// field
																							// list
					fields[fieldIndex].setTableID(tables[table2Index].getNumFigure()); // tell
																						// the
																						// field
																						// what
																						// table
			...
			} else if (fieldIndex >= 0) { // field has already been assigned to
											// a table
	...
	public void parseSaveFile() throws IOException { // this method is fucked
		...
		while (currentLine.startsWith("Table: ")) {
			...
			currentLine = br.readLine(); // this should be the RelatedTables
											// list
			...
			currentLine = br.readLine(); // this should be the RelatedFields
											// list
			...
			currentLine = br.readLine(); // this should be "}"
			currentLine = br.readLine(); // this should be "\n"
			currentLine = br.readLine(); // this should be either the next
											// "Table: ", #Fields#
		}
		while ((currentLine = br.readLine()) != null) {
	...
	private void makeArrays() { // convert ArrayList objects into arrays of the
